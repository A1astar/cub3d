...
/*   Updated: 2025/05/13 11:28:05 by alacroix         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/cub3d.h"

static bool	player_detected(t_enemy *enemy, t_player *player)
{
	float	dx;
	float	dy;
	float	distance;
...
static void	enemy_chase_mode(t_enemy *enemy, t_player *player, t_map *map)
{
	float	next_x;
	float	next_y;
...
static void	enemy_patrol_mod(t_enemy *enemy, t_map *map)
{
	float	next_pos;
...
static void	update_enemy_pos(t_enemy *enemy, t_player *player, t_map *map)
{
	if (enemy->state == dead)
		return ;
...
void	update_enemy(t_cub3d *cub3d)
{
	...
	while (enemy_id < cub3d->nb_enemy)
		update_enemy_pos(&cub3d->randy[enemy_id++], &cub3d->player,
			&cub3d->map);
}
